...
function addCryptoSold(c) {
  event.preventDefault();
  try {
    let check = JSON.parse($("#nameSell").val())[1]; //se esiste la crypto
...
function price() {
  var coin = document.getElementById("name");
  var qnt = document.getElementById("quantity");
  var date = dob.value;

  // transform date to format dd-mm-yyyy
  date = date.split("-").reverse().join("-");
  var coin_name = JSON.parse(coin.value)[0].toLowerCase();
  ...
  $.ajax({
    ...
    dataType: "json",
    success: function (data) {
      // if the field market_data exists
      if (data.market_data) {
        // if the field current_price exists
        if (data.market_data.current_price) {
          // if the field eur exists
          if (data.market_data.current_price.eur) {
            // get the price
            var singlePrice = data.market_data.current_price.eur;
            var price = singlePrice * qnt.value;
            // set the price
...
function priceSell() {
  var coin = document.getElementById("nameSell");
  var qnt = document.getElementById("quantityS");
  var date = dos.value;
      ...
      if (data.market_data) {
        ...
        if (data.market_data.current_price) {
          ...
          if (data.market_data.current_price.eur) {
            // get the price
            var singlePrice = data.market_data.current_price.eur;
            var price = singlePrice * qnt.value * -1;
            // set the price
